...
void generateUnitImplByAttackId(const game::CMidgardID* attackId)
{
    ...
    CMidgardID unitId{};
    id.fromParts(&unitId, category, categoryIndex, IdType::UnitGenerated, typeIndex);

    CUnitGenerator* unitGenerator = (*(GlobalDataApi::get().getGlobalData()))->unitGenerator;
    unitGenerator->vftable->generateUnitImpl(unitGenerator, &unitId);
}
...
bool attackHasPower(game::IAttack* attack)
{
    using namespace game;

    auto id = attack->vftable->getAttackClass(attack)->id;
    const auto& attackClasses = AttackClassCategories::get();
    return id == attackClasses.paralyze->id || id == attackClasses.petrify->id
           || id == attackClasses.damage->id || id == attackClasses.drain->id
           || id == attackClasses.drainOverflow->id || id == attackClasses.fear->id
           || id == attackClasses.lowerDamage->id || id == attackClasses.lowerInitiative->id
           || id == attackClasses.poison->id || id == attackClasses.frostbite->id
           || id == attackClasses.blister->id || id == attackClasses.bestowWards->id
           || id == attackClasses.shatter->id || id == attackClasses.revive->id
           || id == attackClasses.drainLevel->id || id == attackClasses.transformOther->id;
}
                               ...
                               int attackPowerTotal)
{
    ...
    int attackPowerModified = fn.applyPercentModifiers(attackPower, modifiers,
                                                       ModifierElementTypeFlag::Power);
    if (attackPowerModified > 100)
        attackPowerModified = 100;

    return appendColoredBonus(fmt::format("{:d}%", attackPowerModified),
                                          ...
                                          int damageMax)
{
    ...
    auto dmg = appendColoredBonus(fmt::format("{:d}", damageModified),
                                  damageTotalBoosted - damageModified);
    return damageTotalBoosted < damageMax ? dmg : getAttackDamageMaxText(dmg);
}
                                    ...
                                    int lowerInitiativeLevel)
{
    ...
    const auto& fn = gameFunctions();
    int initiativeModified = fn.applyPercentModifiers(initiative, modifiers,
                                                      ModifierElementTypeFlag::Initiative);
    if (initiativeModified > 150)
        initiativeModified = 150;
    else if (initiativeModified < 1)
        initiativeModified = 1;
...
std::string getInfiniteText()
{
    ...
    auto text = getTranslatedText("X005TA1000"); // Reserved for custom text
    if (text.length())
        return text;

    auto langToken = getTranslatedText("X005TA0829"); // "%ATTACK% or %BLANK%"
    if (langToken.find("или") != langToken.npos)
        return "Длительно";
...
std::string getCritHitText()
{
    ...
    auto text = getTranslatedText("X005TA1001"); // Reserved for custom text
    if (text.length())
        return text;
                       ...
                       const game::IdList* modifiers)
{
    ...
    if (altAttack != nullptr && attackHasPower(altAttack)) {
